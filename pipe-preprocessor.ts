export interface PipePreprocessorOptions {
	...
	generateDTS?: boolean;
	ideMode?: boolean;
	// Add option to suppress IDE warnings
...
export function createIDEFriendlyPipePreprocessor(options: PipePreprocessorOptions = {}): {
	markup: (params: { content: string; filename: string }) => PreprocessorResult;
} {
	const {
		pipePrefix = '',
		pipeRegistry = null,
	...
	return {
		markup({ content, filename }: { content: string; filename: string }): PreprocessorResult {
...
function transformForIDE(content: string, filename: string, options: PipePreprocessorOptions): PreprocessorResult {
	...
	const transformedContent = content.replace(pipeRegex, (match, expression, pipeChain) => {
		try {
			...
			// For IDE mode, add a comment to suppress warnings if requested
			if (options.suppressIDEWarnings) {
				return `{/* @ts-ignore */ ${transformed.slice(1, -1)}}`;
			}
...
function transformForBuild(content: string, filename: string, options: PipePreprocessorOptions): PreprocessorResult {
	// More precise regex that only matches actual pipe syntax
	// Must have a valid expression before the pipe and a valid pipe name after
	const pipeRegex = /\{([^{}]+?)\s*\|\s*([^{}]+?)\}/g;
...
function transformPipeExpression(expression: string, pipeChain: string, options: PipePreprocessorOptions): string {
